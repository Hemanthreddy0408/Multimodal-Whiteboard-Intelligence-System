...
# ══════════════════════════════════════════════════════════════
def load_trocr():
    section("Loading TrOCR (Microsoft OCR model)")
    try:
        from transformers import TrOCRProcessor, VisionEncoderDecoderModel
        pprint("  ⏳ Downloading/loading TrOCR weights (first run ~1 min)...")
        t0 = time.time()
...
def load_dinov2():
    section("Loading DINOv2 (Facebook visual embedding model)")
    try:
        import torch
        from transformers import AutoImageProcessor, AutoModel
        pprint("  ⏳ Downloading/loading DINOv2 weights (first run ~30 s)...")
        t0 = time.time()
...
def evaluate():
    ...
    if not EVAL_DIR.exists():
        pprint(f"❌ Evaluation dataset not found at {EVAL_DIR}")
    ...
    if clf is None:
        pprint("❌ Cannot continue without Classifier Head.")
        return
    ...
    for category in CATEGORIES:
        ...
        files = sorted(cat_dir.glob("*.png"))[:5]  # max 5 per class
    ...
    pprint(f"    ├── TrOCR OCR                 : {avg_ocr:.3f}s")
    pprint(f"    ├── DINOv2 Embedding          : {avg_emb:.3f}s")
...
| **OCR Availability** | Available | {"✅ Running" if avg_ocr > 0 else "❌ Unavailable"} | — |
| **DINOv2 Availability** | Available | {"✅ Running" if avg_emb > 0 else "❌ Unavailable"} | — |
...
"""
    ...
    pprint(f"   📊 Accuracy={accuracy:.1f}% | Avg Latency={avg_total:.2f}s")
